//
//  AddDamagedPhotosController.m
//  WlAuction
//
//  Created by LB 3 Mac Mini on 11/02/16.
...
@interface AddDamagedPhotosController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIScrollViewDelegate>
{
...
@implementation AddDamagedPhotosController
...
- (void)viewDidLoad {
    ...
    [self.tabBarController setTitle:@"ADD DAMAGE PHOTOS"];
...
//    if([VehicleDetails sharedInstance].damaged_pictures.count > 0) {
...
-(void)viewDidAppear:(BOOL)animated {
    ...
    self.tabBarController.title = @"ADD DAMAGE PHOTOS";
    ...
    UIBarButtonItem *rightBarButton = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"true_icon"] style:UIBarButtonItemStylePlain target:self action:@selector(loadHistory:)];
    ...
    if([VehicleDetails sharedInstance].damaged_pictures.count > 0 && !_isLoaded) {
//        [WSLoading showLoading];
...
-(void)addImages {
    ...
    int j = 0;
    totalImages = (int)[VehicleDetails sharedInstance].damaged_pictures.count;
    for (ImageInfo *tempInfo in [VehicleDetails sharedInstance].damaged_pictures) {
        ...
        j++;
        if([VehicleDetails sharedInstance].damaged_pictures.count < 20) {
            
            [_imageScroll setContentSize:CGSizeMake(scrollFrame.size.width * j, scrollFrame.size.height)];
            [self.buttonScroll setContentSize:CGSizeMake((tempButton.frame.size.width+20) * [VehicleDetails sharedInstance].damaged_pictures.count, tempButton.frame.size.height)];
            [self.buttonScroll setScrollEnabled:YES];
            [WSLoading dismissLoading];
        }
        
        if(j == [VehicleDetails sharedInstance].damaged_pictures.count) {
    ...
    imgCount = (int)[VehicleDetails sharedInstance].damaged_pictures.count;
    totalImages = (int)[VehicleDetails sharedInstance].damaged_pictures.count;
...
-(void)loadHistory:(id)sender {
    AddCarHistoryViewController *addCarHistory = [self.storyboard instantiateViewControllerWithIdentifier:@"AddCarHistoryViewController"];
    [self.navigationController pushViewController:addCarHistory animated:YES];
}
...
-(IBAction)btnAddImageClicked:(UIButton *)sender {
    ...
    [alertAction addAction:[UIAlertAction actionWithTitle:@"Gallery" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self openGallery];
    }]];
...
#pragma mark - UIIamagePickerController Delegate
